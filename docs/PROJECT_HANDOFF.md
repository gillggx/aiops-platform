...
| **L2 Investigation** | `get_process_info` | 範圍調查 + 自動 SPC charts。`objectName` 參數篩選 SPC/DC/APC/RECIPE，回應扁平化，`objectName=SPC` 自動產生 `_charts`（5 種 chart DSL）。**一次呼叫取代過去 8 次呼叫。** |
...
**MCP v3 效率提升**：`get_process_info` + `objectName=SPC` 一次呼叫自動產生 5 種 SPC chart DSL，取代過去 Agent 需要 8 次獨立 MCP 呼叫的流程（1 step instead of 8）。
...
- **`/process/info`** — L2 範圍調查 + 自動 SPC chart DSL 生成
