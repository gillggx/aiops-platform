...
#### Interactive Skill Authoring（對話式 Skill 建立）— NEW

傳統的 one-shot「填表 → 一次生成」流程改為**多輪對話 session**，讓使用者與 Agent 來回釐清需求、試跑、給 feedback、反覆迭代，直到 Skill 正確為止。

**Skill 建立流程（multi-turn dialog）：**

```
使用者描述需求
  → drafting（建立 session）
    → clarifying   （Agent 產出 understanding + checklist + ambiguities + questions，使用者逐輪 respond）
      → planned    （Phase 1~3 LLM 生成 steps_mapping + input_schema + output_schema）
        → tested   （mock payload try-run，看到即時結果）
          → reviewed（使用者評分 ✅ correct / 🤔 partial / ❌ wrong + comment）
            ├─ feedback=correct → saved（promote 為 skill_definitions）
            └─ feedback=wrong/partial → revising → 回到 planned（LLM 依 diagnosis + fix_summary 重新生成）
```

**三個入口共用同一個 `<SkillAuthoringChat>` 元件：**

| 頁面 | 按鈕 | target_type | 帶入的 context |
|------|------|-------------|----------------|
| `/admin/my-skills` | 🤖 對話建立 Skill | `my_skill` | — |
| `/admin/auto-patrols` | 🤖 對話建立 | `auto_patrol` | `patrol_context`（event type / trigger 條件） |
| `/admin/skills` | 🤖 對話建立 | `diagnostic_rule` | `alarm_context`（alarm type / severity） |

舊的表單流程保留為「+ 表單建立」fallback 入口。

**UI 特色：** 60% chat + 40% 即時 PreviewPanel（左側對話，右側即時顯示 `input_schema` / `output_schema` / `steps_mapping` / 最近 try-run 結果）。每個對話 turn 以不同顏色卡片區分：clarification（黃）、code_generated（藍）、test_result（紫）、code_revised（綠）、generation_failed（紅）。

**後端：** 新增 `skill_authoring_sessions` table 儲存完整對話歷史與 state machine 狀態；新增 `SkillAuthoringService` orchestrator；新增 `/api/v1/skill-authoring` router（8 個 endpoint：`sessions` CRUD + `clarify` SSE + `respond` + `generate` SSE + `try-run` + `feedback` + `revise` SSE + `save`）。Frontend 用 `/api/admin/skill-authoring/[...path]` catch-all proxy 轉發，SSE streams 直接透傳。
...
- `skill_authoring_sessions` — Interactive Skill Authoring 的多輪對話 session（state machine + turns history）
- `users` — 使用者帳號
